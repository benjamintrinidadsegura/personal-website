...
const moduleCopy = {
  en: {
    ...
    available: [
      ...
      { title: "Life Vision", purpose: "Explore desired directions, protected priorities, real constraints and intentionally open possibilities.", audience: "For my future direction", statusLabel: "Available · Beta", privacy: "Local only · up to 20 derived snapshots in this browser", duration: "6 sections · about 10–14 minutes" },
    ],
  ...
  es: {
    ...
    available: [
      ...
      { title: "Life Vision", purpose: "Explora direcciones deseadas, prioridades protegidas, límites reales y posibilidades que quieres mantener abiertas.", audience: "Para mi dirección futura", statusLabel: "Disponible · Beta", privacy: "Solo local · hasta 20 instantáneas derivadas en este navegador", duration: "6 secciones · unos 10–14 minutos" },
    ],
  ...
  tr: {
    ...
    available: [
      ...
      { title: "Life Vision", purpose: "İstenen yönleri, korunacak öncelikleri, gerçek kısıtları ve bilinçli olarak açık bırakılan olasılıkları keşfet.", audience: "Gelecekteki yönüm için", statusLabel: "Kullanılabilir · Beta", privacy: "Yalnızca yerel · bu tarayıcıda en fazla 20 türetilmiş anlık görüntü", duration: "6 bölüm · yaklaşık 10–14 dakika" },
    ],
  ...
  pl: {
    ...
    available: [
      ...
      { title: "Life Vision", purpose: "Zbadaj pożądane kierunki, chronione priorytety, realne ograniczenia i świadomie otwarte możliwości.", audience: "Dla mojego przyszłego kierunku", statusLabel: "Dostępne · Beta", privacy: "Tylko lokalnie · do 20 wyprowadzonych migawek w tej przeglądarce", duration: "6 części · około 10–14 minut" },
    ],
  ...
  el: {
    ...
    available: [
      ...
      { title: "Life Vision", purpose: "Διερεύνησε επιθυμητές κατευθύνσεις, προστατευμένες προτεραιότητες, πραγματικούς περιορισμούς και σκόπιμα ανοιχτές δυνατότητες.", audience: "Για τη μελλοντική μου κατεύθυνση", statusLabel: "Διαθέσιμο · Beta", privacy: "Μόνο τοπικά · έως 20 παραγόμενα στιγμιότυπα σε αυτό το πρόγραμμα περιήγησης", duration: "6 ενότητες · περίπου 10–14 λεπτά" },
    ],
  ...
  ru: {
    ...
    available: [
      ...
      { title: "Life Vision", purpose: "Исследуйте желаемые направления, защищаемые приоритеты, реальные ограничения и намеренно открытые возможности.", audience: "Для моего будущего направления", statusLabel: "Доступно · Beta", privacy: "Только локально · до 20 производных снимков в этом браузере", duration: "6 разделов · около 10–14 минут" },
    ],
...
export function getLifeAlignmentHubContent(locale: Locale) {
  ...
  return {
    ...
    }).map((module) => module.id === "career" ? { ...module, title: careerModule.title[locale], purpose: careerModule.description[locale] } : module) as readonly AvailableLifeAlignmentModule[],
    future: futureLifeAlignmentModules.map((module) => { const index = futureOrder.indexOf(module.id as (typeof futureOrder)[number]); return copy && index >= 0 ? { ...module, purpose: copy.futurePurpose[index], audience: copy.futureAudience[index], statusLabel: copy.comingLater } : module; }) as readonly FutureLifeAlignmentModule[],
  };
}
