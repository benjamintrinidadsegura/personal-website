      ...
      <div className="mx-auto max-w-6xl">
        ...
        {error ? (
          ...
          <ol className="grid gap-4">
            {feedback.map((item) => (
              <li key={item.id}>
                <Link href={`/admin/feedback/${item.id}`} className="grid min-w-0 gap-5 border border-white/15 bg-white/[0.02] p-5 transition hover:border-[#35d0e5]/55 focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-[#35d0e5] sm:p-7 lg:grid-cols-[10rem_1fr_auto]">
                  <div>
                    <p className="font-mono text-xs font-black uppercase tracking-[0.16em] text-[#35d0e5]">{item.status}</p>
                    <p className="mt-2 text-sm text-slate-500">{sourceLabels[item.source_context]}</p>
                  </div>
                  <div className="min-w-0">
                    <h2 className="font-black text-white">{item.name || "Name not provided"}</h2>
                    <p className="mt-3 break-words leading-7 text-slate-300 [overflow-wrap:anywhere]">{item.message_preview}</p>
                  </div>
                  <time dateTime={item.created_at} className="font-mono text-xs text-slate-500">{dateFormatter.format(new Date(item.created_at))}</time>
                </Link>
              </li>
            ))}
          </ol>
