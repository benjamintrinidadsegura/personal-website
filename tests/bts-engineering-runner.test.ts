...
test("sequential uncheckpointed applies preserve exact recent migration evidence", async () => {
  const names = ["20260101000000_first.sql", "20260102000000_second.sql"];
  const root = makeRunnerRepo(names);
  try {
    const firstHash = sha256(readFileSync(join(root, "supabase/migrations", names[0])));
    mkdirSync(join(root, ".bts-engineering/state"), { recursive: true });
    writeFileSync(join(root, ".bts-engineering/state/preflight.json"), JSON.stringify({
      schemaVersion: 1,
      target: { projectRef: TARGET.projectRef },
      applied: { name: names[0], hash: firstHash, appliedAt: new Date().toISOString() },
    }));
    const calls: string[][] = [];
    const execute = async (_command: string, args: string[]) => {
      calls.push(args);
      if (_command === "git") return ok(`?? supabase/migrations/${names[0]}\n?? supabase/migrations/${names[1]}\n`);
      const joined = args.includes("--file") ? readFileSync(args.at(-1)!, "utf8") : args.join(" ");
      if (joined.includes("BTS_TARGET_PROBE")) return ok(`BTS_TARGET_PROBE|postgres|postgres\nBTS_MIGRATION|20260101000000\n`);
      if (args.includes("--dry-run")) return ok("Finished supabase db push.\n", `Would push these migrations:\n • ${names[1]}\n`);
      return ok("BTS_ENGINEERING_RESIDUE_ZERO\n");
    ...
    const preflight = await runner.preflight();
    assert.equal(preflight.status, "migration-required");
    let state = JSON.parse(readFileSync(join(root, ".bts-engineering/state/preflight.json"), "utf8"));
    assert.deepEqual(state.recentApplies.map(({ name }: { name: string }) => name), [names[0]]);
    const result = await runner.applyMigration({ confirmation: `APPLY bts-online-dev ${names[1]}` });
    assert.equal(result.applied, names[1]);
    assert.equal(calls.filter(isMigrationApply).length, 1);
    state = JSON.parse(readFileSync(join(root, ".bts-engineering/state/preflight.json"), "utf8"));
    assert.deepEqual(state.recentApplies.map(({ name }: { name: string }) => name), names);
  } finally {
    rmSync(root, { recursive: true, force: true });
  }
});
