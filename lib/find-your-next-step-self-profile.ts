...
const profileCopy = {
  de: { contextual: (v: string) => `Situationsabhängig zeigt sich zusätzlich: ${v}`, co: (a: string, b: string) => `${a} und ${b} zeigen sich jeweils klar und eigenständig in mehreren Bereichen. Die Kombination wurde jedoch nicht direkt als gemeinsames Muster gewählt und bleibt deshalb eine mögliche redaktionelle Lesart.`, tension: "Beide Signale sind sichtbar und erscheinen in deinem bestehenden Ergebnis bereits als Spannungsfeld. Diese Verbindung bleibt eine mögliche Lesart deiner aktuellen Situation, keine dauerhafte Identität.", pair: "Beide Signale sind sichtbar und wurden mindestens einmal direkt gemeinsam gewählt. Weitere Hinweise aus deiner Momentaufnahme stützen diese Profil-Linse.", incomplete: "Die Profil-Linse entsteht erst, wenn alle Reflexionsfragen beantwortet sind.", sparse: "Deine Antworten zeigen diesmal einzelne Hinweise, aber kein ausreichend klares gemeinsames Muster für eine Profil-Linse. FYNS lässt die Zuordnung deshalb bewusst offen – auch das ist ein valides Ergebnis.", unsupported: "In deiner Momentaufnahme sind einzelne Themen erkennbar, aber keine der kuratierten Profil-Linsen ist ausreichend belegt.", mixed: "Deine Momentaufnahme ist gerade vielseitiger als eindeutig. Mehrere Profil-Linsen wären ähnlich gut begründbar; deshalb stellt FYNS bewusst keine davon an die erste Stelle." },
  en: { contextual: (v: string) => `Depending on the situation, an additional signal appears: ${v}`, co: (a: string, b: string) => `${a} and ${b} each appear clearly and independently in several areas. However, you did not select the combination directly as a shared pattern, so it remains one possible editorial reading.`, tension: "Both signals are visible and already appear as a tension in your existing result. The connection remains one possible reading of your current situation, not a permanent identity.", pair: "Both signals are visible and were selected together directly at least once. Further indications in your snapshot support this profile lens.", incomplete: "The profile lens is built only after all reflection questions have been answered.", sparse: "Your answers show individual indications this time, but no sufficiently clear shared pattern for a profile lens. FYNS deliberately leaves the assignment open — that is a valid result too.", unsupported: "Individual themes are visible in your snapshot, but none of the curated profile lenses has enough support.", mixed: "Your snapshot is currently more varied than singular. Several profile lenses would be supported similarly well, so FYNS deliberately does not place one of them first." },
  es: { contextual: (v: string) => `Según la situación, aparece además: ${v}`, co: (a: string, b: string) => `${a} y ${b} aparecen con claridad y de forma independiente en varias áreas. Como no elegiste directamente la combinación, sigue siendo una posible lectura editorial.`, tension: "Ambas señales son visibles y ya aparecen como tensión en tu resultado. La conexión sigue siendo una posible lectura de tu situación actual, no una identidad permanente.", pair: "Ambas señales son visibles y se eligieron juntas directamente al menos una vez. Otros indicios de tu instantánea apoyan esta lente.", incomplete: "La lente de perfil se forma cuando hayas respondido todas las preguntas de reflexión.", sparse: "Tus respuestas muestran indicios sueltos, pero no un patrón conjunto suficientemente claro. FYNS deja la asignación abierta de forma consciente; también es un resultado válido.", unsupported: "En tu instantánea se reconocen temas individuales, pero ninguna de las lentes seleccionadas tiene apoyo suficiente.", mixed: "Tu instantánea es ahora más variada que unívoca. Varias lentes tendrían un apoyo parecido, así que FYNS no coloca ninguna en primer lugar." },
  tr: { contextual: (v: string) => `Duruma bağlı olarak ek bir sinyal görünüyor: ${v}`, co: (a: string, b: string) => `${a} ve ${b} birkaç alanda açık ve bağımsız biçimde görünüyor. Birleşimi doğrudan seçmediğin için bu, olası bir editoryal okuma olarak kalır.`, tension: "İki sinyal de görünür ve mevcut sonucunda zaten bir gerilim oluşturuyor. Bu bağlantı kalıcı bir kimlik değil, güncel durumuna dair olası bir okuma olarak kalır.", pair: "İki sinyal de görünür ve en az bir kez doğrudan birlikte seçildi. Anlık görüntündeki diğer işaretler bu merceği destekliyor.", incomplete: "Profil merceği tüm düşünme soruları yanıtlandıktan sonra oluşur.", sparse: "Yanıtların bazı işaretler gösteriyor, ancak profil merceği için yeterince net ortak örüntü yok. FYNS eşleştirmeyi bilinçli olarak açık bırakır; bu da geçerli bir sonuçtur.", unsupported: "Anlık görüntünde ayrı temalar seçiliyor, ancak düzenlenmiş profil merceklerinden hiçbiri yeterince desteklenmiyor.", mixed: "Anlık görüntün şu anda tek bir örüntüden daha çeşitli. Birkaç mercek benzer ölçüde gerekçelendirilebilir; FYNS bu yüzden hiçbirini öne koymaz." },
  pl: { contextual: (v: string) => `Zależnie od sytuacji pojawia się też: ${v}`, co: (a: string, b: string) => `${a} i ${b} są wyraźnie i niezależnie widoczne w kilku obszarach. Nie wybrano jednak ich bezpośrednio jako wspólnego wzorca, więc pozostają możliwą redakcyjną interpretacją.`, tension: "Oba sygnały są widoczne i już pojawiają się w wyniku jako napięcie. To połączenie pozostaje możliwą interpretacją obecnej sytuacji, a nie trwałą tożsamością.", pair: "Oba sygnały są widoczne i co najmniej raz wybrane bezpośrednio razem. Dalsze wskazówki w migawce wspierają tę soczewkę.", incomplete: "Soczewka profilu powstaje dopiero po odpowiedzi na wszystkie pytania refleksyjne.", sparse: "Odpowiedzi pokazują pojedyncze wskazówki, ale nie dość wyraźny wspólny wzorzec. FYNS świadomie pozostawia przypisanie otwarte — to również prawidłowy wynik.", unsupported: "W obrazie widać pojedyncze tematy, lecz żadna z wybranych soczewek nie ma wystarczającego wsparcia.", mixed: "Twój obraz jest teraz bardziej różnorodny niż jednoznaczny. Kilka soczewek miałoby podobne uzasadnienie, dlatego FYNS żadnej nie stawia na pierwszym miejscu." },
  el: { contextual: (v: string) => `Ανάλογα με την κατάσταση, εμφανίζεται επιπλέον: ${v}`, co: (a: string, b: string) => `${a} και ${b} εμφανίζονται καθαρά και ανεξάρτητα σε πολλούς τομείς. Δεν επέλεξες όμως άμεσα τον συνδυασμό ως κοινό μοτίβ, οπότε παραμένει μια πιθανή επιμελητική ανάγνωση.`, tension: "Και τα δύο σήματα είναι ορατά και εμφανίζονται ήδη ως ένταση στο αποτέλεσμα. Η σύνδεση παραμένει μια πιθανή ανάγνωση της τρέχουσας κατάστασης, όχι μόνιμη ταυτότητα.", pair: "Και τα δύο σήματα είναι ορατά και επιλέχθηκαν άμεσα μαζί τουλάχιστον μία φορά. Πρόσθετες ενδείξεις στο στιγμιότυπο στηρίζουν τον φακό.", incomplete: "Ο φακός προφίλ σχηματίζεται αφού απαντηθούν όλες οι ερωτήσεις αναστοχασμού.", sparse: "Οι απαντήσεις δείχνουν μεμονωμένες ενδείξεις, αλλά όχι αρκετά καθαρό κοινό μοτίβ. Το FYNS αφήνει συνειδητά την αντιστοίχιση ανοιχτή· και αυτό είναι έγκυρο αποτέλεσμα.", unsupported: "Στο στιγμιότυπο διακρίνονται μεμονωμένα θέματα, αλλά κανένας επιμελημένος φακός δεν στηρίζεται αρκετά.", mixed: "Το στιγμιότυπό σου είναι τώρα πιο ποικίλο παρά μονοσήμαντο. Πολλοί φακοί θα στηρίζονταν εξίσου, οπότε το FYNS δεν δίνει προτεραιότητα σε κανέναν." },
  ru: { contextual: (v: string) => `В зависимости от ситуации также проявляется: ${v}`, co: (a: string, b: string) => `${a} и ${b} ясно и независимо проявляются в нескольких областях. Но сочетание не было выбрано напрямую как общий паттерн, поэтому остаётся возможной редакционной интерпретацией.`, tension: "Оба сигнала видны и уже проявляются в результате как напряжение. Эта связь остаётся возможным прочтением текущей ситуации, а не постоянной идентичностью.", pair: "Оба сигнала видны и хотя бы один раз выбраны непосредственно вместе. Дополнительные признаки в снимке поддерживают эту линзу.", incomplete: "Профильная линза формируется после ответа на все вопросы для размышления.", sparse: "В ответах есть отдельные признаки, но нет достаточно ясного общего паттерна. FYNS сознательно оставляет сопоставление открытым — это тоже допустимый результат.", unsupported: "В снимке заметны отдельные темы, но ни одна из подготовленных линз не получила достаточной поддержки.", mixed: "Сейчас твой снимок скорее разнообразный, чем однозначный. Несколько линз обоснованы почти одинаково, поэтому FYNS сознательно не ставит одну из них первой." },
} as const;
...
function buildWhy(candidate: ProfileCandidate, locale: Locale): string {
  ...
  return candidate.basis === "tension" ? copy.tension : copy.pair;
}
