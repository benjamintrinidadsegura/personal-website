create type public.feedback_result_product as enum (
  'personal-advantage',
  'money-profile'
);

create type public.feedback_result_fit as enum (
  'mostly',
  'partly',
  'not_really'
);

create type public.feedback_usefulness_category as enum (
  'pattern',
  'tradeoffs',
  'next_steps',
  'stress_context',
...
alter table public.private_feedback
  ...
  drop constraint private_feedback_message_check,
  add column result_product public.feedback_result_product,
  add column result_fit public.feedback_result_fit,
  add column usefulness_category public.feedback_usefulness_category,
  add column locale text,
  add constraint private_feedback_message_check
    check (
      message is null
      or (
  ...
  add constraint private_feedback_locale_check
    check (locale is null or locale in ('de', 'en', 'es', 'tr', 'pl', 'el', 'ru')),
  add constraint private_feedback_result_context_check
    check (
      (
        result_product is null
        and result_fit is null
        and usefulness_category is null
        and message is not null
      )
      or (
        result_product is not null
        and result_fit is not null
        and name is null
        and contact_method is null
        and contact_value is null
        and source_context = 'other'
        and (message is null or pg_catalog.char_length(message) <= 1200)
      )
    );

drop function public.submit_private_feedback(
  text,
  text,
...
create function public.submit_private_feedback(
  p_message text,
  p_name text,
  p_contact_method public.feedback_contact_method,
  p_contact_value text,
  p_source_context public.feedback_source_context,
  p_result_product public.feedback_result_product,
  p_result_fit public.feedback_result_fit,
  p_usefulness_category public.feedback_usefulness_category,
  p_locale text,
  p_network_hash text,
...
begin
  if p_source_context is null
    ...
    or (
      ...
      and (
        p_message <> pg_catalog.btrim(p_message)
        ...
        or exists (
          ...
          where pg_catalog.strpos(p_message, pg_catalog.chr(unsafe.code_point)) > 0
        )
      )
    )
    or (
    ...
    or (
      ...
      and (
        ...
        or exists (
          ...
          where pg_catalog.strpos(p_contact_value, pg_catalog.chr(unsafe.code_point)) > 0
        )
      )
    )
    or (
      p_result_product is null
      and (p_message is null or p_result_fit is not null or p_usefulness_category is not null)
    )
    or (
      p_result_product is not null
      and (
        p_result_fit is null
        or p_name is not null
        or p_contact_method is not null
        or p_contact_value is not null
        or p_source_context <> 'other'
        or (p_message is not null and pg_catalog.char_length(p_message) > 1200)
      )
    )
  then
...
returns table (
  ...
  result_product public.feedback_result_product,
  result_fit public.feedback_result_fit,
  usefulness_category public.feedback_usefulness_category,
  locale text,
...
revoke all on function public.submit_private_feedback(
  ...
  text,
  public.feedback_source_context,
  public.feedback_result_product,
  public.feedback_result_fit,
  public.feedback_usefulness_category,
  text,
  text,
  text
