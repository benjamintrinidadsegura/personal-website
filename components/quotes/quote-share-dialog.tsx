...
export function QuoteShareDialog({ copy, onClose, quote, safeSharePath, socialPostEnabled = false, surfaceLabel }: { copy: QuoteDictionary; onClose: () => void; quote: SelectedQuote; safeSharePath: string; socialPostEnabled?: boolean; surfaceLabel: string }) {
  ...
  const card = style === "socialPost"
    ? <QuoteSocialPostCard format={format} quote={quote} originalLabel={copy.original} surfaceLabel={surfaceLabel} />
    : <QuoteShareCard format={format} quote={quote} originalLabel={copy.original} surfaceLabel={surfaceLabel} />;
        ...
        <div className="writing-share-composer">
          ...
          <div className="writing-share-composer-grid">
            ...
            <aside className="writing-share-controls">
              ...
              {socialPostEnabled ? <ShareStyleSelector label={copy.style} labels={copy.styles} value={style} onChange={(value) => { setStyle(value); setFeedback(null); }} /> : null}
              ...
              <ShareFileActions key={`${quote.id}:${style}:${format}`} cardRef={cardRef} fileName={`bts-quote-${quote.id}${style === "socialPost" ? "-social-post" : ""}-${format}`} format={format} renderKey={`${quote.id}:${style}:${format}`} text={shareText} title={copy.dialogTitle} url={shareUrl} />
