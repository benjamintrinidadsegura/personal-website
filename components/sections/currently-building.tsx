  ...
  return (
    <section id="building" aria-labelledby="building-title" className="section-lines px-5 py-24 sm:px-8 sm:py-32">
      <Reveal className="mx-auto max-w-[90rem]">
        <div id="building-title"><SectionHeading eyebrow="Currently Building / 06" title={copy.sectionTitle} description={copy.sectionDescription} accent="orange" /></div>
        <div className="mt-16 grid auto-rows-[minmax(20rem,auto)] gap-5 lg:grid-cols-12">
          {projects.map((project, index) => (
            <article key={project.slug} style={{ "--project-accent": project.accent } as CSSProperties} className={`project-card pattern-${project.pattern} group relative overflow-hidden rounded-[1.75rem] border border-white/10 bg-[#091d2c] transition duration-300 hover:-translate-y-1 hover:border-[var(--project-accent)] focus-within:border-[var(--project-accent)] ${project.featured ? "lg:col-span-7" : "lg:col-span-5"}`}>
              <Link href={localizeHref(`/projects/${project.slug}`, locale)} aria-label={`${project.name} – ${copy.openProjectLabel}`} className="flex h-full min-h-80 flex-col p-7 focus:outline-none sm:p-9">
                <div className="relative z-10 flex items-start justify-between gap-4">
                  ...
                  <span className="rounded-full border border-white/15 bg-[#061521]/75 px-3 py-1 font-mono text-[10px] uppercase tracking-wider text-slate-200">{copy.statusLabels[project.status]}</span>
                </div>
                <div className="relative z-10 mt-auto max-w-2xl pt-16">
                  ...
                  <span className="mt-7 inline-flex items-center font-bold text-[var(--project-accent)]">{copy.enterProject} <span aria-hidden="true" className="ml-2 transition group-hover:translate-x-1">↗</span></span>
                </div>
              </Link>
            </article>
          ))}
