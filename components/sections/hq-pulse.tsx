...
export async function HqPulse({ publishedWriting = [] }: { publishedWriting?: readonly PublicWritingSummary[] }) {
  ...
  const items = localizeHqPulseItems(createHqPulseItems({ publishedWriting }), locale);
              ...
              >
                ...
                >
                  <div className="flex flex-wrap items-center justify-between gap-3">
                    ...
                    <div className="flex flex-wrap justify-end gap-2">{featured ? <span className="rounded-full border px-3 py-1 font-mono text-[10px] uppercase tracking-[0.14em]" style={{ borderColor: `${accent}80`, color: accent }}>{copy.newest}</span> : null}{item.status ? <span className="rounded-full border border-white/15 px-3 py-1 font-mono text-[10px] uppercase tracking-[0.14em] text-slate-300">{item.status}</span> : null}</div>
                  </div>
                  ...
                  <div className="mt-auto pt-16">
                    ...
                    <div className="mt-8 flex flex-wrap items-center justify-between gap-4 border-t border-white/10 pt-5">
                      <span className="font-mono text-xs uppercase tracking-[0.16em] text-slate-500">{item.date ? <time dateTime={item.date}>{dateFormatter.format(new Date(item.date))}</time> : item.source ? <>{copy.source} / {item.source}</> : null}</span>
