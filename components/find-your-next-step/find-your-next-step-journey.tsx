...
import { fynsFoundationUiCopy } from "@/data/find-your-next-step-ui-locales";
import { StatusPill } from "@/components/ui/status-pill";
import type { Locale } from "@/lib/i18n/config";
import { localizeHref } from "@/lib/i18n/routing";
import type { NextStepJourney } from "@/types/find-your-next-step";

export function FindYourNextStepJourneyBreadcrumb({ journey, locale }: { journey: NextStepJourney; locale: Locale }) {
  const findYourNextStep = getFindYourNextStep(locale);
  const copy = fynsFoundationUiCopy[locale];
  return (
    <nav aria-label={copy.breadcrumb} className="font-mono text-xs text-slate-400">
      <ol className="flex flex-wrap items-center gap-2">
        <li>
          <Link href={localizeHref("/", locale)} className="inline-flex min-h-11 items-center transition hover:text-white focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-[var(--journey-accent)]">
            Digital HQ
          </Link>
        </li>
        <li aria-hidden="true">/</li>
        <li>
          <Link href={localizeHref(findYourNextStep.href, locale)} className="inline-flex min-h-11 items-center transition hover:text-white focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-[var(--journey-accent)]">
            {findYourNextStep.name}
          </Link>
        </li>
        <li aria-hidden="true">/</li>
        <li aria-current="page" className="text-[var(--journey-accent)]">{copy.path} {journey.number}</li>
      </ol>
    </nav>
  );
}

export function FindYourNextStepJourney({ journey, locale }: { journey: NextStepJourney; locale: Locale }) {
  const findYourNextStep = getFindYourNextStep(locale);
  const copy = fynsFoundationUiCopy[locale];
  return (
    <article
    ...
    >
      ...
      <div className="relative mx-auto max-w-6xl">
        ...
        <header className="grid min-h-[64svh] items-center gap-12 border-b border-white/15 py-16 lg:grid-cols-[1.12fr_0.88fr] lg:py-24">
          <div>
            <p className="font-mono text-xs font-black uppercase tracking-[0.28em] text-[var(--journey-accent)]">
              Find Your Next Step / {copy.path} {journey.number}
            </p>
          ...
          <aside aria-label={copy.status} className="border-l border-[var(--journey-accent)] pl-7 sm:pl-9">
            <StatusPill>{journey.status}</StatusPill>
            <h2 className="mt-6 text-2xl font-black leading-snug text-white sm:text-3xl">{copy.building}</h2>
        ...
        <section aria-labelledby="journey-expectations-title" className="grid gap-10 border-b border-white/15 py-20 lg:grid-cols-[0.38fr_1fr] sm:py-24">
          <p className="font-mono text-xs uppercase tracking-[0.22em] text-[var(--journey-accent)]">{copy.expect}</p>
          <div>
            <h2 id="journey-expectations-title" className="text-3xl font-black text-white sm:text-5xl">{copy.expectTitle}</h2>
        ...
        <section aria-labelledby="journey-analysis-title" className="grid gap-10 border-b border-white/15 py-20 lg:grid-cols-[0.38fr_1fr] sm:py-24">
          <div>
            <p className="font-mono text-xs uppercase tracking-[0.22em] text-[var(--journey-accent)]">{copy.later}</p>
            <p className="mt-5 max-w-xs text-sm leading-6 text-slate-500">{copy.noScore}</p>
          </div>
          <div>
            <h2 id="journey-analysis-title" className="text-3xl font-black text-white sm:text-5xl">{copy.signals}</h2>
        ...
        {journey.professionalBoundary ? (
          <aside aria-labelledby="professional-boundary-title" className="grid gap-6 border-b border-white/15 py-14 lg:grid-cols-[0.38fr_1fr]">
            <p className="font-mono text-xs uppercase tracking-[0.2em] text-slate-500">{copy.context}</p>
            <div>
              <h2 id="professional-boundary-title" className="text-xl font-black text-white">{copy.limits}</h2>
        ...
        <div className="flex flex-col items-start justify-between gap-6 py-14 sm:flex-row sm:items-center">
          <p className="max-w-xl text-slate-500">{copy.foundation}</p>
          <Link
