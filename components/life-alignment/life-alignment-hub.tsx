...
function ActiveModuleCard({ module, index }: { module: AvailableLifeAlignmentModule; index: number }) {
  ...
  return (
    <li>
      <Link
        ...
        className="group relative flex h-full min-h-[30rem] flex-col overflow-hidden rounded-[1.75rem] border border-white/10 p-6 transition-[border-color,transform] motion-safe:duration-300 motion-safe:hover:-translate-y-1 motion-reduce:transform-none hover:border-[var(--module-accent)]/60 focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-[var(--module-accent)] sm:p-8"
      >
        ...
        <div className="relative flex flex-1 flex-col">
          ...
          <dl className="mt-8 grid gap-3 border-t border-white/10 pt-6 text-sm leading-6">
            <div>
              <dt className="font-mono text-[10px] font-black uppercase tracking-[0.16em] text-slate-500">Für wen</dt>
              <dd className="mt-1 text-slate-200">{module.audience}</dd>
            </div>
            <div>
              <dt className="font-mono text-[10px] font-black uppercase tracking-[0.16em] text-slate-500">Privatsphäre</dt>
              <dd className="mt-1 text-slate-200">{module.privacy}</dd>
            </div>
            <div>
              <dt className="font-mono text-[10px] font-black uppercase tracking-[0.16em] text-slate-500">Umfang</dt>
          ...
          <span className="mt-8 inline-flex min-h-11 items-center font-mono text-xs font-black uppercase tracking-[0.18em] text-[var(--module-accent)]">
            Perspektive öffnen <span aria-hidden="true" className="ml-2 transition-transform motion-safe:group-hover:translate-x-1">→</span>
          </span>
        </div>
      </Link>
    </li>
...
export function LifeAlignmentHub() {
  return (
    <article className="section-lines relative overflow-hidden px-5 pb-24 pt-28 sm:px-8 sm:pt-36">
      <div
        aria-hidden="true"
        className="absolute inset-x-0 top-0 h-[58rem] bg-[radial-gradient(circle_at_82%_12%,rgba(245,185,113,0.15),transparent_28rem),radial-gradient(circle_at_15%_34%,rgba(116,216,200,0.08),transparent_24rem),radial-gradient(circle_at_58%_60%,rgba(185,165,255,0.07),transparent_24rem)]"
      />
      ...
      <div className="relative mx-auto max-w-[90rem]">
        ...
        <header className="grid min-h-[66svh] items-center gap-14 border-b border-white/15 py-16 lg:grid-cols-[1.08fr_0.92fr] lg:py-24">
          ...
          <aside aria-labelledby="life-modes-title" className="border-l border-[#f5b971] pl-7 sm:pl-9">
            <p id="life-modes-title" className="font-mono text-xs uppercase tracking-[0.22em] text-[#f5b971]">Drei Blickrichtungen</p>
            <ol className="mt-7 grid gap-5">
              {availableLifeAlignmentModules.map((module, index) => (
                <li key={module.id} className="grid grid-cols-[2rem_1fr] gap-4 border-b border-white/10 pb-5">
        ...
        <section aria-labelledby="life-active-title" className="border-b border-white/15 py-20 sm:py-28">
          <div className="grid gap-6 lg:grid-cols-[0.36fr_1fr] lg:gap-12">
            ...
            <div>
              <h2 id="life-active-title" className="text-4xl font-black tracking-[-0.04em] text-white sm:text-6xl">Wähle die Perspektive, die zu deiner Frage passt.</h2>
              <p className="mt-6 max-w-3xl text-lg leading-8 text-slate-300">Die drei Module teilen eine Haltung, aber nicht dieselben Fragen oder Ergebnisse.</p>
            </div>
          </div>

          <ol className="mt-14 grid gap-6 lg:grid-cols-3">
            {availableLifeAlignmentModules.map((module, index) => <ActiveModuleCard key={module.id} module={module} index={index} />)}
          </ol>
        </section>
        ...
        <section aria-labelledby="life-future-title" className="border-b border-white/15 py-20 sm:py-24">
          <div className="grid gap-8 lg:grid-cols-[0.44fr_1fr] lg:gap-14">
            <div>
              <p className="font-mono text-xs font-black uppercase tracking-[0.22em] text-slate-500">Teil der Produktfamilie</p>
              <h2 id="life-future-title" className="mt-5 text-3xl font-black text-white sm:text-5xl">Weitere Kontexte kommen später.</h2>
              <p className="mt-5 max-w-xl leading-7 text-slate-400">Diese Perspektiven gehören zur verbindlichen Richtung von Life Alignment. Ihre Journeys sind in V1 bewusst noch nicht verfügbar.</p>
            </div>

            <ul className="grid gap-3 sm:grid-cols-2" aria-label="Spätere Life-Alignment-Module">
              {futureLifeAlignmentModules.map((module) => (
                <li key={module.id} className="rounded-2xl border border-dashed border-white/15 bg-[#061521]/45 p-5">
                  <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <section aria-labelledby="life-principle-title" className="grid gap-10 py-20 lg:grid-cols-[0.36fr_1fr] sm:py-24">
          <p className="font-mono text-xs font-black uppercase tracking-[0.22em] text-[#f5b971]">Gemeinsame Haltung</p>
          <div>
            <h2 id="life-principle-title" className="text-3xl font-black text-white sm:text-5xl">Verstehen statt vermessen.</h2>
            <p className="mt-6 max-w-4xl text-lg leading-8 text-slate-300">{lifeAlignmentHub.principle}</p>
            <ul className="mt-9 grid gap-3 sm:grid-cols-3">
              {[
                "Kein Life- oder Kompatibilitätsscore",
                "Keine Diagnose oder richtige Antwort",
                "Antworten bleiben im jeweiligen lokalen Seitenzustand",
