...
export function HumanContextResultActions({ copyText, printTitle, accent, eyebrow, title, description, copyLabel, copiedMessage, manualCopyLabel, printMarker = "" }: HumanContextResultActionsProps) {
  ...
  const handleCopy = async () => {
    ...
    setFeedback(copied ? { kind: "success", message: copiedMessage } : { kind: "error", message: "Automatisches Kopieren hat nicht geklappt. Nutze bitte das Textfeld." });
  ...
  return (
    <section aria-labelledby="human-context-actions-title" data-fyns-result-actions className="mt-16 border-t border-white/15 pt-12">
      ...
      <div className="mt-7 flex flex-col gap-3 sm:flex-row sm:flex-wrap">
        <button type="button" onClick={handlePrint} className="min-h-12 rounded-full px-6 py-3 font-black text-[#07131d] focus-visible:outline-2 focus-visible:outline-offset-4" style={{ backgroundColor: accent, outlineColor: accent }}>Drucken / als PDF speichern</button>
        <button type="button" disabled={copying} onClick={handleCopy} className="min-h-12 rounded-full border border-white/20 px-6 py-3 font-bold text-white transition hover:border-white/45 disabled:opacity-60 focus-visible:outline-2 focus-visible:outline-offset-4" style={{ outlineColor: accent }}>{copying ? "Wird kopiert …" : copyLabel}</button>
      </div>
