...
function AccountContents({
  ...
  const articleMatch = pathname.match(/^\/writing\/([a-z0-9]+(?:-[a-z0-9]+)*)$/u);
  ...
  if (account.kind === "anonymous") {
    return <Link className={itemClass} href="/account/login" onClick={onNavigate}>Log in</Link>;
  }
    ...
    <>
      {account.kind === "admin" ? (
        <div className="border-b border-white/10 pb-2">
          ...
          {account.aal === "aal1" ? (
            <Link className={itemClass} href="/admin/mfa" onClick={onNavigate}>Verify to access BTS Studio</Link>
          ) : (
            <>
              {articleMatch ? <Link className={`${itemClass} text-[#35d0e5]`} href={`/admin/writing/by-slug/${articleMatch[1]}`} onClick={onNavigate}>Edit this article</Link> : null}
              ...
              <Link className={itemClass} href="/admin/echowall" onClick={onNavigate}>EchoWall Moderation</Link>
            </>
          )}
        </div>
      ) : null}
      <form action={logoutAction} className="pt-2">
        <button className={itemClass}>Logout</button>
      </form>
    </>
  );
}
...
export function AccountMenu({
  ...
  if (mobile) {
    return (
      <section aria-labelledby="mobile-account-title" className="mt-6 border-t border-white/10 pt-6">
        <p id="mobile-account-title" className="px-3 font-mono text-xs font-black uppercase tracking-[0.25em] text-[#35d0e5]">BTS Account</p>
  ...
  return (
    <div ref={wrapper} className="relative hidden lg:block">
      ...
      >
        <p className="px-4 pb-2 pt-2 text-sm font-black text-white">BTS Account</p>
